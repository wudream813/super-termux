...
int create_about_pane(void) {
    ...
    int len = snprintf(about_buf, sizeof(about_buf),
        ...
        /* ★ 这行平台无关，所以不放 platform.h。原来副标题写「高性能单文件 C 终端复用
         *   多标签环境」，「单文件 C」会被读成「源码只有一个 C 文件」（实际 20 个 .c +
         *   18 个 .h、约 1.9 万行）；真正成立的是【构建产物】是单个可执行文件。
         *   合起来写是 76 格，超过关于页方框的 64 格会折行，所以拆成两行。 */
        "  \x1b[038;2;139;148;158m纯 C 实现，单文件可执行\x1b[0m\r\n\r\n"
