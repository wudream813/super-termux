...
当前版本：**v2.0.9**（正式支持 Windows / Linux / macOS 三个系统）
...
- **图形化设置页**：`Ctrl+B s` 打开，侧栏含「启动 / 菜单项 / 外观·主题 / 键位 / 行为 / 窗格配色」六类；主题可即时预览切换、语义色可改十六进制、键位支持**按键录制**，改完即时写回 `termux.ini`
...
| **窗格配色** | cmd / shell 文字的**默认背景、默认前景**、**滚动条滑块 / 轨道**与黑/红/绿/黄/蓝/紫/青/白及 8 个亮色共 20 项（即 `[theme] pane_*`），两列排布，`↑/↓` 选、`←/→` 换列，`Enter` 输 6 位 hex（预填值一打字就整段替换），`R` 复位当前项回「跟随终端」，`Ctrl+R` 全部清除；改完立即生效并写盘。命令面板里也有 `pane-palette` 入口 | 启动页按 `F5` / `W` |
