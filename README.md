...
当前版本：**v2.1.0**（正式支持 Windows / Linux / macOS 三个系统）
...
- **图形化设置页**：`Ctrl+B s` 打开，侧栏含「启动 / 菜单项 / 外观·主题 / 键位 / 行为 / 窗格配色」六类；主题可即时预览切换、语义色可改十六进制、键位支持**按键录制**，改完即时写回 `termux.ini`。窗口再小也不缺内容：矮终端下每页可上下滚动（提示行标出行窗口），窄终端下装不下的文字截断为 `...` 并**鼠标悬停浮出完整内容**
