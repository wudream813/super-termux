...
| 回归判据 | `tests/verify_pane_palette.py` 新增 **P 组**：40 列（纯键盘档）与 56 列（有按钮档）各验「不折行、不盖侧栏、Shift+滚轮真的改变画面、纵向行窗口标记不变」；O/K/N/M 组的入口按键随新页面改掉（`[改]` 的点击改从条目管理页进，详情页经 `[M]` → `Enter` 进）。把整套判据拿去跑 **v2.1.3** 的二进制 ⇒ 恰好且只有 K1、N1 与 3 条 P 变红（新行为的判据确实咬得住，不是放宽判据掩盖回归）。另在 `cols=28..60 × rows=10/12/18/24/30 × 6 个子页 × 滚轮 0/3` 全档校验「没有任何一行超过终端宽度」 |
