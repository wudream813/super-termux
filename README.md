...
当前版本：**v2.0.7**（正式支持 Windows / Linux / macOS 三个系统）
...
| **窗格配色** | cmd / shell 文字的**默认背景、默认前景**与黑/红/绿/黄/蓝/紫/青/白及 8 个亮色共 18 项（即 `[theme] pane_*`），两列排布，`↑/↓` 选、`←/→` 换列，`Enter` 输 6 位 hex（预填值一打字就整段替换），`R` 复位当前项回「跟随终端」，`Ctrl+R` 全部清除；改完立即生效并写盘。命令面板里也有 `pane-palette` 入口 | 启动页按 `F5` / `W` |
