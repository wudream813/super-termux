...
当前版本：**v2.1.4**（正式支持 Windows / Linux / macOS 三个系统）
...
| 侧栏分类 | 能做什么 | 快捷键 |
|---|---|---|
| 启动 (Startup) | **只读**的启动项表（名称 / 命令行随窗口宽度显示，超出的部分 Shift+滚轮看）＋「设为启动默认」单选项；`Enter`/`E`/`M`/数字键把选中的那条带去「条目管理」页，双击直接进它的详细配置 | `↑/↓` 选、`←/→`/`Space` 切默认、`T` 终端、`H` 帮助 |
| **[M] 条目管理** | 增删改与排序都在这里：顶部 `[+] 新建条目` / `[P] 预设库`，每行 `[↑] [↓] [改] [删]`（`[改]` 与 `Enter` 进该条详细配置，`X` 删除），`Ctrl+↑/↓` 直接调序 | 启动页按 `M`，或侧栏点这一项 |
...
窗口很窄时右侧表格装不下所有列：**Shift + 滚轮 = 横向滚动**（普通滚轮照旧翻行），行尾 `«n/m»`
表示横到底没有，聚焦的那一行会自动滚进视野；按钮 `[↑][↓][改][删]` 钉在视口右端，窄到放不下时
按「丢命令行 → 收窄名称列 → 丢 `[↑][↓]` → 丢 `[改][删]`」的顺序让位（丢到底仍可用键盘操作）。
